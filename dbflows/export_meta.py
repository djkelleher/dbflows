...
from .utils import copy_to_csv, date_range, default_logger, schema_table as get_schema_table, split_schema_table
...
class ExportMeta:
    ...
    def from_file(cls, path: Union[str, Path]):
        ...
        if slice_start := params["slice_range"]:
            slice_start, slice_end = slice_start.split("_")
    ...
    def create_file_name(self) -> str:
        ...
        if self.slice_start or self.slice_end:
            ...
            # TODO date type.
            if isinstance(self.slice_start, datetime):
                assert isinstance(self.slice_end, datetime)
                stem_parts.append(
...
def target_export_row_count(
    ...
    if row_bytes is not None:
        ...
        logger.info(
            "Determining export row count for table %s. Using sample size %f",
            table.name,
            sample_n_rows,
        )
    ...
    if slice_column is not None:
        ...
        if (
            ...
            existing_table_exports = [
                ...
                if f.prefix == file_stem_prefix and f.schema_table == str(schema_table)
            ]
